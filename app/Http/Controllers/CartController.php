...
class CartController extends Controller
{
    ...
    public function addDraftItem(Request $request, $draftKey, Product $product)
    {
           $item = new Item($product, $request->qty);
            ...
            if($qty > $stok){
                flash('Maaf Stok Kurang')->important()->error();
                return redirect()->route('cart.index');              
            }
    ...
    public function store(Request $request, $draftKey)
    {
        ...
        foreach($draft->items as $r){
            ...
            $product = Product::find($product_id); // Mengambil produk dengan ID 1

            if ($product) {
                $stockToReduce = $qty; // Jumlah stok yang akan dikurangi
            
                $currentStock = $product->stok;
                $newStock = $currentStock - $stockToReduce;
